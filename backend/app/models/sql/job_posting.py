...
class JobPosting(Base):
    ...
    # The employer's own website (Apify ``companyWebsite``). Unlike ``source_url``
    # (always a LinkedIn URL), this carries the company's real domain, which the
    # cold-outreach node uses for an accurate Hunter.io lookup.
    #
    # New column — regenerate the schema with Alembic after changing this model:
    #   alembic revision --autogenerate -m "add_company_website"
    ...
    # Cold-outreach contact resolved by the find_recruiter_contact node (Hunter.io)
    # during matching; null when no named recruiter was found. Persisted so the
    # "send to recruiter" action has a recipient even after a page reload.
    #
    # New columns — regenerate the schema with Alembic after changing this model:
    #   alembic revision --autogenerate -m "add_recruiter_contact"
    #   alembic upgrade head
